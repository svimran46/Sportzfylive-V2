...
async function syncBroadcastData(env) {
  ...
  const matches = await readArray("matches");
  const loadedChannels = await readArray("channels");
  const channels = loadedChannels
    .filter(channel => channel.autoGenerated !== true)
    .map(channel => ({
      ...channel,
      id: channel.id || slugify(channel.name) || slugify(channel.url)
    }));

  // One-time cleanup of TV Sources created by the old broadcast automation.
  // Manually/imported channels are preserved because they do not have
  // autoGenerated === true.
  if (channels.length !== loadedChannels.length) {
    await env.SPORTZFY_DB.put("channels", JSON.stringify(channels));
  }

  // Broadcast Sync only links broadcasts to TV Sources that already exist.
  // It never creates new TV Source records. Existing channels are matched by
  // broadcast name/aliases, with country used as a disambiguator when present.
